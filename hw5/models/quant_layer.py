...
class weight_quantize_fn(nn.Module):
    def __init__(self, w_bit):
        ...
        self.register_parameter('wgt_alpha', Parameter(torch.tensor(3.0)))  ## weight quantization alpha == 3
...
class QuantConv2d(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=False):
        ...
        self.bit = 2   ## might change to 2-bit
        self.weight_quant = weight_quantize_fn(w_bit=self.bit)
        self.act_alq = act_quantization(self.bit)
        self.act_alpha = torch.nn.Parameter(torch.tensor(8.0))  ## activation quantization alpha == 8, would be optimized as a training parameter
    ...
    def forward(self, x):
        ...
        # Store weight_q during the training to reuse for the hardware design
        x = self.act_alq(x, self.act_alpha)  ## quantized input activation
